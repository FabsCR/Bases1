...
# Rutas para los reportes con las vistas

@app.route('/reporte_libros', methods=['GET'])
def get_reporte_libros():
    try:
        db = get_db_connection()
        cursor = db.cursor()
        ...
        inventario = [
            {
                'IDLibro': row[0],
                'TituloLibro': row[1],
                'CantidadLibros': row[2],
                'Status': row[3],
                'FechaAdquisicion': row[4],
                'Editorial': row[5]
            }
            for row in rows
        ]
